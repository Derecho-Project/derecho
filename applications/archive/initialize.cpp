...
std::map<uint32_t, std::pair<ip_addr_t, port_t>> initialize(const uint32_t num_nodes) {
    // Get conf file parameters
    // Global information
    const std::string leader_ip = getConfString(CONF_DERECHO_LEADER_IP);
    const port_t leader_gms_port = getConfUInt16(CONF_DERECHO_LEADER_GMS_PORT);
    // Local information
    const uint32_t local_id = getConfUInt32(CONF_DERECHO_LOCAL_ID);
    const std::string local_ip = getConfString(CONF_DERECHO_LOCAL_IP);
    const port_t sst_port = getConfUInt16(CONF_DERECHO_SST_PORT);
    const port_t gms_port = getConfUInt16(CONF_DERECHO_GMS_PORT);

    bool is_leader = (leader_ip == local_ip && leader_gms_port == gms_port);

    if(is_leader) {
        // The map from id to IP and SST Port.
        std::map<uint32_t, std::pair<ip_addr_t, port_t>> ip_addrs_and_ports{{local_id, {local_ip, sst_port}}};
        ...
        std::map<uint32_t, std::pair<ip_addr_t, port_t>> ip_addrs_and_ports;
