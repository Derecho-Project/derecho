...
SubView::SubView(int32_t num_members)
    : mode(Mode::ORDERED), members(num_members), is_sender(num_members, 1),
      member_ips_and_ports(num_members), joined(0), departed(0), my_rank(-1) {}

SubView::SubView(
    Mode mode, const std::vector<node_id_t> &members,
    std::vector<int> is_sender,
    const std::vector<std::tuple<ip_addr_t, uint16_t, uint16_t, uint16_t,
                                 uint16_t>> &member_ips_and_ports)
    : mode(mode), members(members), is_sender(members.size(), 1),
      member_ips_and_ports(member_ips_and_ports), my_rank(-1) {
...
View::View(
    ...
    : vid(vid), members(members),
      member_ips_and_ports(member_ips_and_ports), failed(failed),
      num_failed(0), joined(joined), departed(departed),
      num_members(members.size()), my_rank(my_rank),
...
int View::rank_of(const std::tuple<ip_addr_t, uint16_t, uint16_t, uint16_t,
                                   uint16_t> &who) const {
  for (int rank = 0; rank < num_members; ++rank) {
...
SubView View::make_subview(const std::vector<node_id_t> &with_members,
  ...
  std::vector<std::tuple<ip_addr_t, uint16_t, uint16_t, uint16_t, uint16_t>>
      subview_member_ips_and_ports(with_members.size());
  for (std::size_t subview_rank = 0; subview_rank < with_members.size();
       ++subview_rank) {
    std::size_t member_pos =
        std::distance(members.begin(), std::find(members.begin(), members.end(),
                                                 with_members[subview_rank]));
    if (member_pos == members.size()) {
      // The ID wasn't found in members[]
      throw subgroup_provisioning_exception();
    }
    subview_member_ips_and_ports[subview_rank] =
        member_ips_and_ports[member_pos];
  }
  // Note that joined and departed do not need to get initialized here; they
  // will be initialized by ViewManager
  return SubView(mode, with_members, is_sender,
                 subview_member_ips_and_ports);
}

int View::subview_rank_of_shard_leader(subgroup_id_t subgroup_id,
                                       int shard_index) const {
  const SubView &shard_view =
      subgroup_shard_views.at(subgroup_id).at(shard_index);
  ...
  for (int n = 0; n < my_rank; n++) {
    for (int row = 0; row < my_rank; row++) {
      if (!failed[n] && !gmsSST->suspected[row][n]) {
        return false; // I'm not the new leader, or some failure suspicion
...
void View::merge_changes() {
  ...
  for (int n = 0; n < num_members; n++) {
    ...
    if (gmsSST->num_committed[myRank] <
        gmsSST->num_committed[n]) // How many I know to have been committed
    {
  ...
  for (int n = 0; n < num_members; n++) {
    ...
    if (!found) {
      gmssst::set(gmsSST->changes[myRank][gmsSST->num_changes[myRank] %
                                          gmsSST->changes.size()],
                  members[n]);
