...
mutils::KindMap<Factory, Types...> make_kind_map(Factory<Types>... factories) {
    ...
    kind_map_builder<decltype(factories_map), Types...>(factories_map,
                                                        factories...);
...
Group<ReplicatedTypes...>::Group(const CallbackSet &callbacks,
        ...
        : whenlog(logger(create_logger()), )
          ...
          view_manager([&]() {
              if(is_starting_leader) {
                  ...
                  return ViewManager(leader_connection.value(), callbacks, subgroup_info,
                                     persistence_manager.get_callbacks(),
                                     _view_upcalls);
              }
          }()),
    ...
    if(!is_starting_leader) {
        std::set<std::pair<subgroup_id_t, node_id_t>> subgroups_and_leaders = construct_objects<ReplicatedTypes...>(
...
Group<ReplicatedTypes...>::construct_objects(
    ...
    for(uint32_t subgroup_index = 0; subgroup_index < subgroup_ids.size();
        ++subgroup_index) {
        ...
        for(uint32_t shard_num = 0; shard_num < num_shards; ++shard_num) {
            ...
            if(std::find(members.begin(), members.end(), my_id) != members.end()) {
                ...
                if(old_object != replicated_objects.template get<FirstType>().end() && old_object->second.get_shard_num() != shard_num) {
                    whenlog(
                            logger->debug("Deleting old Replicated Object state for type {}; "
                                          "I was reassigned from shard {} to shard {}",
                                          typeid(FirstType).name(),
                                          old_object->second.get_shard_num(), shard_num));
                    replicated_objects.template get<FirstType>().erase(old_object);
                }
                // If we don't have a Replicated<T> for this (type, subgroup index), we
                // just became a member of the shard
                if(replicated_objects.template get<FirstType>().count(
                           subgroup_index)
                   == 0) {
                    // Determine if there is existing state for this shard that will need
                    // to be received
                    bool has_previous_leader = old_shard_leaders && old_shard_leaders->size() > subgroup_id && (*old_shard_leaders)[subgroup_id].size() > shard_num && (*old_shard_leaders)[subgroup_id][shard_num] > -1 && (*old_shard_leaders)[subgroup_id][shard_num] != my_id;
                    if(has_previous_leader) {
                        subgroups_to_receive.emplace(
                                subgroup_id, (*old_shard_leaders)[subgroup_id][shard_num]);
                    }
             ...
             * be received from the leader and there are no logs to update */
                        replicated_objects.template get<FirstType>().emplace(
                                subgroup_index,
                                Replicated<FirstType>(my_id, subgroup_id, subgroup_index,
                                                      shard_num, rpc_manager, this));
                    } else {
                        replicated_objects.template get<FirstType>().emplace(
                                subgroup_index,
                                Replicated<FirstType>(
                                        my_id, subgroup_id, subgroup_index, shard_num, rpc_manager,
                                        factories.template get<FirstType>(), this));
                    }
                    // Store a reference to the Replicated<T> just constructed
                    objects_by_subgroup_id.emplace(
                            subgroup_id,
        ...
        if(!in_subgroup) {
            ...
            if(old_object != replicated_objects.template get<FirstType>().end()) {
                whenlog(logger->debug(
                        "Deleting old Replicated Object state (of type {}) for subgroup {} "
                        "because this node is no longer a member",
    ...
    return functional_insert(
            subgroups_to_receive,
            construct_objects<RestTypes...>(curr_view, old_shard_leaders));
}

template <typename... ReplicatedTypes>
std::vector<RawSubgroup>
Group<ReplicatedTypes...>::construct_raw_subgroups(const View &curr_view) {
...
void Group<ReplicatedTypes...>::set_up_components() {
    ...
    curr_view.get().multicast_group->register_rpc_callback(
            [this](subgroup_id_t subgroup, node_id_t sender, char *buf,
                   uint32_t size) {
    ...
    view_manager.register_send_object_upcall([this](subgroup_id_t subgroup_id,
        ...
        whenlog(logger->debug(
                "Sending Replicated Object state for subgroup {} to node {}",
                subgroup_id, new_node_id));
        subgroup_object.send_object(joiner_socket.get());
    });
    view_manager.register_initialize_objects_upcall(
            [this](node_id_t my_id, const View &view,
                   const vector_int64_2d &old_shard_leaders) {
                // ugh, we have to copy the vector to get it as a pointer
                std::set<std::pair<subgroup_id_t, node_id_t>> subgroups_and_leaders = construct_objects<ReplicatedTypes...>(
                        view, std::make_unique<vector_int64_2d>(old_shard_leaders));
                receive_objects(subgroups_and_leaders);
...
Group<ReplicatedTypes...>::create_logger() const {
    ...
    log->set_level(whendebug(spdlog::level::off) whenrelease(spdlog::level::off));
...
Group<ReplicatedTypes...>::receive_old_shard_leaders(
...
template <typename... ReplicatedTypes>
template <typename SubgroupType>
Replicated<SubgroupType> &
Group<ReplicatedTypes...>::get_subgroup(SubgroupType *,
                                        uint32_t subgroup_index) {
...
auto &Group<ReplicatedTypes...>::get_subgroup(uint32_t subgroup_index) {
    if(!view_manager.get_current_view().get().is_adequately_provisioned) {
        throw subgroup_provisioning_exception(
                "View is inadequately provisioned because subgroup provisioning "
                "failed!");
    }
...
template <typename... ReplicatedTypes>
template <typename SubgroupType>
ExternalCaller<SubgroupType> &
Group<ReplicatedTypes...>::get_nonmember_subgroup(uint32_t subgroup_index) {
    try {
        return external_callers.template get<SubgroupType>().at(subgroup_index);
    } catch(std::out_of_range &ex) {
        throw invalid_subgroup_exception(
                "No ExternalCaller exists for the requested subgroup; this node may be "
...
template <typename... ReplicatedTypes>
template <typename SubgroupType>
ShardIterator<SubgroupType>
Group<ReplicatedTypes...>::get_shard_iterator(uint32_t subgroup_index) {
    try {
        ...
        for(uint i = 0; i < shard_subviews.size(); ++i) {
            // for shard iteration to be possible, each shard must contain at least
            // one member
...
template <typename... ReplicatedTypes>
void Group<ReplicatedTypes...>::receive_objects(
        const std::set<std::pair<subgroup_id_t, node_id_t>>
                &subgroups_and_leaders) {
    // This will receive one object from each shard leader in ascending order of
    // subgroup ID
    for(const auto &subgroup_and_leader : subgroups_and_leaders) {
        LockedReference<std::unique_lock<std::mutex>, tcp::socket> leader_socket = rpc_manager.get_socket(subgroup_and_leader.second);
        ...
        if(subgroup_object.is_persistent()) {
            ...
            whenlog(logger->debug(
                    "Sending log tail length of {} for subgroup {} to node {}.",
                    log_tail_length, subgroup_and_leader.first,
                    subgroup_and_leader.second));
