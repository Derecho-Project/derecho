...
P2PConnections::P2PConnections(const node::NodeCollection& members, size_t max_payload_size, size_t window_size, const msg::recv_upcall_t& receive_upcall)
    ...
    for(auto other_rank : members.other_ranks) {
        node::NodeCollectionWithSenders pair_of_nodes(other_rank < members.my_rank ? std::vector<node::node_id_t>{members.nodes[other_rank], members.my_id}
                                                                                   : std::vector<node::node_id_t>{members.my_id, members.nodes[other_rank]},
...
P2PConnections::P2PConnections(P2PConnections&& oldP2PConnections, const node::NodeCollection& members)
        : members(std::move(oldP2PConnections.members)),
          connections(members.num_nodes),
  ...
  for(auto other_rank : members.other_ranks) {
    try {
        const uint32_t old_rank = oldP2PConnections.members.get_rank_of(members.nodes[other_rank]);
