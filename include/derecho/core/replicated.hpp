...
public:
    ...
    /**
     * Retrieves an allocator which can be used to prepare a message for 
     * ordered_send_prepared. When its companion ordered_send_prepared is invoked, 
     * it will send a multicast to the entire subgroup that replicates this 
     * Replicated<T>, invoking the RPC function identified by the FunctionTag 
     * template parameter. The caller must keep the returned QueryResults object 
     * in scope in order to receive replies.
     * @param args The arguments to the RPC function
     * @return An instance of rpc::QueryResults<Ret>, where Ret is the return type
     * of the RPC function being invoked.
     */
