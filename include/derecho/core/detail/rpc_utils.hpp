...
public:
     ...
     * a P2P RPC call is not a member of the group that receives the message and a
     * P2P message should not generate a new version anyway.
     */
     ...
     * QueryResults that are generated by ordered_send calls.
     */
     ...
     * work on QueryResults that are generated by ordered_send calls.
     */
...
public:
     ...
     * Blocks until the update caused by this RPC function call has finished
     * persisting on all replicas (i.e. the version number assigned to it has
     * reached the "globally persisted" state).
     */
    void await_global_persistence() {
        global_persistence_done.get();
    }
    ...
    /**
     * Blocks until the update caused by this RPC function call has been signed
     * on all replicas and the signatures have been verified.
     */
    void await_signature_verification() {
        signature_done.get();
    }
...
private:
    ...
    */
    ...
    /**
     * True if global persistence has finished, i.e. set_global_persistence()
     * has been called. This is necessary because it's impossible to ask a
     * std::promise if set_value() has been called on it.
     */
    bool global_persistence_finished = false;
    /**
...
public:
     ...
     * should be called to signal client code that the update has finished
     * persisting on all replicas.
     */
    void set_global_persistence() {
        global_persistence_promise.set_value();
        global_persistence_finished = true;
    }

    /**
     * @return True if global persistence has finished for this update and no
     * more persistence events need to be signaled.
     */
    bool persistence_finished() const {
        return global_persistence_finished;
    }
     ...
     * correctly signed on all replicas.
     */
