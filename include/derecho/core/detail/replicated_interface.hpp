...
public:
    ...
    virtual void sign(openssl::Signer& signer, unsigned char* signature_buffer) = 0;
    virtual const persistent::version_t get_minimum_latest_persisted_version() noexcept(false) = 0;
    virtual void persist(const persistent::version_t version, const unsigned char* signature, std::size_t signature_size) noexcept(false) = 0;
