...
// Note: Inheriting GroupReference is necessary so the RPC method can get the object's current version
...
public:
    ...
    ByteArrayObject(Persistent<test::Bytes>& _p_bytes, uint64_t messages_received, TestState* test_state)
            : pers_bytes(std::move(_p_bytes)), messages_received(messages_received), test_state(test_state) {}
    // default constructor
    ByteArrayObject(PersistentRegistry* pr, TestState* test_state)
            : pers_bytes(pr), messages_received(0), test_state(test_state) {}
...
int main(int argc, char* argv[]) {
    ...
    auto persistence_callback = [&](derecho::subgroup_id_t subgroup, persistent::version_t ver) {
        if(shared_test_state.last_version_set && ver == shared_test_state.last_version) {
            shared_test_state.persist_complete_time = std::chrono::steady_clock::now();
    ...
    double thp_gbps = (static_cast<double>(shared_test_state.total_num_messages) * msg_size) / persist_nanosec;
    double thp_ops = (static_cast<double>(shared_test_state.total_num_messages) * 1000000000) / persist_nanosec;
