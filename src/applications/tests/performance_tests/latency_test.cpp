...
int main(int argc, char* argv[]) {
    ...
    if(num_senders_selector == 0 || (num_senders_selector == 1 && my_rank > (num_nodes - 1) / 2) || (num_senders_selector == 2 && my_rank == num_nodes - 1)) {
        ...
        std::tie(avg_latency, avg_std_dev) = aggregate_latency_tcp(getConfString(CONF_DERECHO_LEADER_IP), my_rank == 0, my_rank, group_members.size(), (average_time / 1000.0), (std_dev / 1000.0));
    } else {
        // if not a sender, then pass 0 as the latency (not counted)
        std::tie(avg_latency, avg_std_dev) = aggregate_latency_tcp(getConfString(CONF_DERECHO_LEADER_IP), my_rank == 0, my_rank, group_members.size(), 0.0, 0.0);
    }
