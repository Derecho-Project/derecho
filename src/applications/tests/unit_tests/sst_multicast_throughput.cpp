...
int main(int argc, char* argv[]) {
    ...
    // compute row_offset, based on the user's selected number of senders
    ...
    // In half-senders and one-sender mode, the lower-ranked nodes are set to non-sending
    ...
    if(node_rank == num_nodes - 1 || num_senders_selector == 0
       || (node_rank > (num_nodes - 1) / 2 && num_senders_selector == 1)) {
        for(uint i = 0; i < num_messages; ++i) {
            ...
            while(buf == NULL) {
                buf = group.get_buffer(max_msg_size);
            }
