...
std::string read_string(socket& s) {
    ...
    size_t size;
    s.read(size);
    // Construct a blank std::string that will hold it
    std::string temp_string(size, ' ');
    // Read the string from the socket into the std::string's buffer
    s.read(&temp_string[0], size);
    return temp_string;
}
