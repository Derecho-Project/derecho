...
int main(int argc, char* argv[]) {
    ...
    for(uint i = 1; i < num_msgs + 1; ++i) {
        auto msg_builder = cookedMessagesHandle.prepare_ordered_send<RPC_NAME(send)>(1024);
        //cookedMessagesHandle.ordered_send<RPC_NAME(send)>(my_rank, i);
        auto fst = msg_builder.template build_arg<0>(my_rank);
        auto scnd = msg_builder.template build_arg<1>(i);
        msg_builder.send(fst, scnd);
    }
    while(!done) {
    }
    if(my_rank == 0) {
