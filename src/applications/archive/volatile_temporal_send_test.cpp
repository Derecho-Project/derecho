...
public:
    ...
    void change_vola_bytes(const MyBytes &bytes) {
        *vola_bytes = bytes;
    }
    ...
    MyBytes query_vola_bytes(uint64_t query_us) const {
        HLC hlc{query_us, 0};
        try {
    ...
    ByteArrayObject(Persistent<MyBytes, ST_MEM> &_v_bytes) :  //  ByteArrayObject(Persistent<MyBytes> & _p_bytes):
                                                            //    pers_bytes(std::move(_p_bytes)) {
                                                           vola_bytes(std::move(_v_bytes)) {
    }
    // the default constructor
    ByteArrayObject(PersistentRegistry *pr) :  //    pers_bytes(nullptr,pr) {
                                              vola_bytes([](){return std::make_unique<MyBytes>();}, nullptr, pr) {
    }
};
