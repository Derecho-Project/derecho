...
void write_string(tcp::socket& s, const std::string str) {
    // We are going to write the null character
    s.write(str.size() + 1);
    s.write(str.c_str(), str.size() + 1);
}

std::string read_string(tcp::socket& s) {
...
std::map<uint32_t, std::pair<ip_addr_t, uint16_t>> initialize(const uint32_t num_nodes) {
    ...
    if(is_leader) {
        ...
        std::map<uint32_t, tcp::socket> id_sockets_map;
        ...
        for(auto& id_socket_pair : id_sockets_map) {
            tcp::socket& s = id_socket_pair.second;
