...
namespace tcp {
bool TCPConnectionManager::add_connection(const node_id_t other_id,
...
void TCPConnectionManager::establish_node_connections(const std::map<node_id_t, std::pair<ip_addr_t, uint16_t>>& ip_addrs_and_ports) {
...
TCPConnectionManager::TCPConnectionManager(node_id_t my_id,
                                 const std::map<node_id_t, std::pair<ip_addr_t, uint16_t>> ip_addrs_and_ports)
        : my_id(my_id) {
    establish_node_connections(ip_addrs_and_ports);
}

void TCPConnectionManager::destroy() {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    sockets.clear();
    conn_listener.reset();
}

bool TCPConnectionManager::write(node_id_t node_id, char const* buffer,
                            size_t size) {
    std::lock_guard<std::mutex> lock(sockets_mutex);
...
bool TCPConnectionManager::write_all(char const* buffer, size_t size) {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    bool success = true;
    for(auto& p : sockets) {
        if(p.first == my_id) {
...
bool TCPConnectionManager::read(node_id_t node_id, char* buffer,
                           size_t size) {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    const auto it = sockets.find(node_id);
    assert(it != sockets.end());
...
bool TCPConnectionManager::add_node(node_id_t new_id, const std::pair<ip_addr_t, uint16_t>& new_ip_addr_and_port) {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    assert(new_id != my_id);
...
bool TCPConnectionManager::delete_node(node_id_t remove_id) {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    return (sockets.erase(remove_id) > 0);
}

bool TCPConnectionManager::contains_node(node_id_t node_id) {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    return (sockets.find(node_id) != sockets.end());
}

int32_t TCPConnectionManager::probe_all() {
    std::lock_guard<std::mutex> lock(sockets_mutex);
    for(auto& p : sockets) {
...
void TCPConnectionManager::filter_to(const std::vector<node_id_t>& live_nodes_list) {
