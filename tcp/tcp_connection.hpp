...
class socket {
    ...
    explicit socket(int _sock, ip_addr_t remote_ip)
...
public:
     ...
     * lookup fails.
     */
    socket(ip_addr_t server_ip, uint16_t server_port);
    socket(socket&& s);
    ...
    bool is_empty() const;
    ip_addr_t get_self_ip();
    ip_addr_t get_remote_ip() const { return remote_ip; }
     ...
     * system call.
     */
    int try_connect(ip_addr_t servername, int port, int timeout_ms = 20000);
